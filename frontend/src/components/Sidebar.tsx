      ...
      {isEditing ? (
        <form onSubmit={onEditSubmit} className="py-1">
          <input
            ...
            className="w-full px-3 py-2 text-sm bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg shadow-sm outline-none font-semibold text-slate-900 dark:text-white"
            onBlur={onEditBlur}
          />
        </form>
      ) : (
        <div
          ...
          className={clsx(
            "w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 border group cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:focus-visible:ring-2 dark:focus-visible:ring-neutral-500",
            isActive || isDragOver
              ? "bg-indigo-50 dark:bg-neutral-800 text-indigo-900 dark:text-neutral-200 border-indigo-200 dark:border-neutral-600 shadow-sm"
              : "text-slate-600 dark:text-neutral-400 border-transparent hover:bg-slate-50 dark:hover:bg-neutral-800 hover:border-gray-200 dark:hover:border-neutral-700"
          )}
        >
  ...
  return (
    <>
      <div className="w-full flex flex-col h-full bg-transparent">
        <div className="p-4 sm:p-5 pb-2">
          <h1 className="text-2xl text-slate-900 dark:text-white flex items-center gap-3 tracking-tight" style={{ fontFamily: 'Excalifont' }}>
            ...
            <span className="text-xs font-semibold text-red-500 mt-2" style={{ fontFamily: 'sans-serif' }}>BETA</span>
          </h1>
        </div>

        <nav
          className="flex-1 overflow-y-auto py-3 sm:py-4 space-y-4 sm:space-y-8 no-scrollbar"
          onContextMenu={handleBackgroundContextMenu}
        >
          <div className="space-y-1">
            <div className="px-6 pb-2 text-[11px] font-semibold text-slate-400 dark:text-neutral-500 uppercase tracking-wider">
              Library
            </div>
            <div className="pl-3 pr-2">
              <button
                ...
                className={clsx(
                  "w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 border",
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between px-6 pb-2 group/header">
              <span className="text-[11px] font-semibold text-slate-400 dark:text-neutral-500 uppercase tracking-wider">Collections</span>
              <button
            ...
            {isCreating && (
              <form onSubmit={handleCreateSubmit} className="mb-2 px-4" onClick={e => e.stopPropagation()}>
                <input
                  ...
                  className="w-full px-3 py-2 text-sm bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg shadow-sm outline-none placeholder:text-slate-400 dark:placeholder:text-neutral-500 font-semibold text-slate-900 dark:text-white"
        ...
        <div className="px-3 pt-3 sm:pt-4 pb-3 sm:pb-4 border-t border-slate-200/50 dark:border-slate-700/50 space-y-2">
          <button
            ...
            className={clsx(
              "w-full flex items-center gap-3 px-3 py-2 text-sm font-semibold rounded-xl transition-all duration-200 border border-gray-200 dark:border-neutral-700 shadow-sm",
              selectedCollectionId === 'trash' || isTrashDragOver
                ? "bg-rose-50 dark:bg-rose-900/30 text-rose-900 dark:text-rose-300"
                : "bg-white dark:bg-neutral-900 text-slate-900 dark:text-neutral-200 hover:bg-rose-50 dark:hover:bg-rose-900/30 hover:text-rose-900 dark:hover:text-rose-300 hover:shadow-md"
            )}
          >
            <Trash2 size={18} />
            ...
            <button
              ...
              className={clsx(
                ...
                selectedCollectionId === 'PROFILE'
                  ? "bg-indigo-50 dark:bg-neutral-800 text-indigo-900 dark:text-neutral-200"
                  : "bg-white dark:bg-neutral-900 text-slate-900 dark:text-neutral-200 hover:bg-slate-50 dark:hover:bg-neutral-800 hover:shadow-md"
              )}
            >
              <User size={18} />
          ...
          {authEnabled && (
            <div className="mt-auto pt-4 border-t border-slate-200 dark:border-neutral-700">
              {user && (
                <div className="py-2 text-xs text-slate-500 dark:text-neutral-500 mb-2">
                  <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-indigo-600 text-white font-semibold flex items-center justify-center">
              ...
              <button
                onClick={logout}
                className="w-full flex items-center gap-3 px-3 py-2 text-sm font-semibold rounded-xl transition-all duration-200 border border-rose-200 dark:border-rose-700 bg-white dark:bg-neutral-900 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/30 hover:shadow-sm cursor-pointer"
              >
                <LogOut size={18} />
      ...
      {contextMenu && (
        ...
        >
          <div
            className="absolute bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700 shadow-md py-1 min-w-[160px] animate-in fade-in zoom-in-95 duration-100"
