  ...
  return (
    <div className={clsx("relative inline-flex items-center", className)} ref={containerRef}>
      <button
        ...
        className={clsx(
          "flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition-all text-sm font-semibold outline-none",
          variant === "bordered" 
            ? "border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-sm active:shadow-none"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[420px] bg-white dark:bg-neutral-900 rounded-[20px] border border-gray-200 dark:border-neutral-700 shadow-lg flex flex-col animate-in fade-in zoom-in-95 duration-200">
        <div className="px-5 py-3.5 flex items-center justify-between border-b-2 border-black/25 dark:border-neutral-700">
          <h2 className="text-basefont-semibold text-slate-800 dark:text-neutral-100 truncate pr-4" title={drawingName}>
            Share "{drawingName}"
          </h2>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg border-[1px] border-transparent hover:border-black/25 dark:hover:border-neutral-600 transition-all group shrink-0"
          >
        ...
        <div className="flex-1 px-5 py-5 space-y-5 overflow-visible">
          {error && (
            <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-900/20 border-[1px] border-rose-600 dark:border-rose-500 text-xs font-semibold text-rose-600 dark:text-rose-400 flex items-center gap-3">
            ...
            {userResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 border border-gray-200 dark:border-neutral-700 rounded-xl bg-white dark:bg-neutral-900 shadow-lg overflow-hidden z-[200] animate-in fade-in slide-in-from-top-2">
                {userResults.map((u) => (
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-700 dark:text-indigo-300font-semibold text-xs border-[1px] border-black/25 dark:border-neutral-600">
                      {u.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xsfont-semibold text-slate-900 dark:text-neutral-100 truncate">{u.name}</div>
          ...
          <section className="space-y-2">
            <h3 className="text-[9px]font-semibold uppercase tracking-[0.1em] text-slate-400 dark:text-neutral-500 px-1">People with access</h3>
            
            <div className="space-y-0">
              <div className="flex items-center gap-3 px-1 py-1.5">
                <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-neutral-800 flex items-center justify-center text-slate-600 dark:text-neutral-300font-semibold text-sm border-[1px] border-black/25 dark:border-neutral-600 shrink-0">
                  {user?.name?.charAt(0).toUpperCase() || "U"}
                </div>
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="text-xsfont-semibold text-slate-900 dark:text-neutral-100 leading-tight">
              ...
              {(sharing?.permissions || []).map((p) => (
                <div key={p.id} className="flex items-center gap-3 px-1 py-1.5 group">
                  <div className="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center text-indigo-600 dark:text-indigo-400font-semibold text-sm border-[1px] border-indigo-600 dark:border-indigo-500 shrink-0">
                    {p.granteeUser.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    <div className="text-xsfont-semibold text-slate-900 dark:text-neutral-100 leading-tight truncate">{p.granteeUser.name}</div>
          ...
          <section className="pt-5 border-t-2 border-black/25 dark:border-neutral-700">
            <h3 className="text-[9px]font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-neutral-500 px-1 mb-3">General access</h3>
            
            <div className="flex items-start gap-4 px-1">
              <div className={clsx(
                "w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border-[1px] transition-all mt-0.5",
                activeLink 
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-0">
                ...
                <p className="text-[11px] font-semibold text-slate-500 dark:text-neutral-400 leading-snug px-0.5">
                  {activeLink 
                ...
                {activeLink && (
                  <div className="pt-3.5 space-y-3.5 animate-in fade-in slide-in-from-top-1 duration-200">
                    <p className="text-[9px]font-semibold text-slate-500 dark:text-neutral-400 px-0.5">
                    ...
                    {expiryOption === "custom" && (
                      <input
                        ...
                        className="w-full px-3 py-1.5 rounded-xl border border-gray-200 dark:border-neutral-700 bg-slate-50 dark:bg-neutral-800 text-[10px]font-semibold focus:outline-none focus:border-indigo-600 transition-all shadow-sm"
                      />
                    )}

                    {linkPermission === "edit" && (
                      <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/10 border-[1px] border-amber-500 space-y-1.5 shadow-[2px_2px_0px_0px_rgba(245,158,11,0.2)]">
                        <div className="flex items-start gap-2">
                           <AlertTriangle size={14} strokeWidth={3} className="text-amber-600 shrink-0 mt-0.5" />
                           <div className="text-[10px] text-amber-900 dark:text-amber-200font-semibold leading-relaxed">
        ...
        <div className="px-5 py-5 flex items-center justify-between border-t-2 border-black/25 dark:border-neutral-700 bg-slate-50 dark:bg-neutral-800/50 rounded-b-[18px]">
          <button
            onClick={() => handleCopy(currentLinkUrl)}
            disabled={!activeLink}
            className={clsx(
              "flex items-center gap-2 px-4 py-2.5 rounded-xl border-[1px]font-semibold text-[10px] uppercase tracking-widest transition-all active:translate-x-[1px] active:translate-y-[1px]",
              isCopied 
                ? "bg-emerald-500 text-white border-black/25 shadow-none translate-x-[1px] translate-y-[1px]" 
                : "bg-white dark:bg-neutral-900 border-black/25 dark:border-neutral-600 text-indigo-600 dark:text-indigo-400 shadow",
          ...
          <button
            onClick={onClose}
            className="px-8 py-2.5 rounded-xl bg-indigo-600 dark:bg-indigo-500 text-white border-[1px] border-black/25 font-semibold text-[10px] uppercase tracking-[0.2em] hover:shadow-md active:translate-y-0 transition-all"
          >
            DONE
          </button>
        </div>
